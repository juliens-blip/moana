...
SECTION_PROMPT_CORE = (
    ...
    "realistic materials, stable gimbal-like motion, and smooth temporal consistency. "
    "Give each shot a purposeful editorial progression: establish the space, execute "
    "one confident move with controlled acceleration and easing, then finish on a "
    "strong reveal or detail that motivates the next cut. Keep it elegant but dynamic, "
    "with no passive drift, flicker, warping, morphing, or cheap slideshow aesthetics. "
    "Depict only what is genuinely visible; never invent or exaggerate yacht features. "
    "If the brochure supplies a brokerage agency logo, preserve its exact design and "
    "spelling as a persistent, static, very light semi-transparent watermark for the "
    "entire clip, around 8-12 percent opacity in a safe corner or quiet negative space. "
    "Never obscure the yacht, redesign or animate the logo, show it full-screen, or "
    "invent a logo that is not supplied. For yacht interiors, build an active premium "
    "room tour in three beats: wide establishing view, decisive forward dolly or "
    "curved lateral track with strong natural parallax, then a meaningful reveal. Arc "
    "around real furniture, move toward a visible doorway or adjacent room, and end "
    "with directional momentum for the next cut. Never use passive hovering, only "
    "zoom-in, zoom-out, static pan, or Ken Burns motion, and never invent unseen rooms. "
    "Use brochure text selectively as refined editorial graphics. On opening identity "
    "imagery, show the exact yacht name during the first moments only when clearly "
    "legible, then fade it cleanly. Later, show exact visible facts such as builder, "
    "model, year, length, performance, accommodation, or location one at a time as "
    "restrained lower-thirds or section cards. Keep typography crisp and correctly "
    "spelled; never invent a name, specification, claim, price, or unreadable text."
)
