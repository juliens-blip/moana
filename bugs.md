...
[2026-08-26] | Routes brochure-video `/api/brochure-video` et `/api/brochure-video/[jobId]/status` retournent 500 en local | Tentative de connexion SSH vers MOANA_SSH_HOST indéfini ; seule MOANA_SSH_KEY existe dans `.env` | Ajouter `MOANA_SSH_HOST=ubuntu@51.44.220.145` (ou votre serveur) à `.env.local` ; redémarrer `npm run dev` | #frontend #backend #deploy #bug #blocked

[2026-08-26] | Service systemd `moana-brochure-video@.service` manquant sur EC2 ubuntu@51.44.220.145 | Factory a généré le template mais dernière session factory n'a pas ré-installé le service sur l'hôte distant | Redéployer via `workers/deploy/deploy.py` (mode `install`) ou SSH manuel ; copier `workers/deploy/moana-brochure-video@.service` vers `/etc/systemd/system/` et `systemctl daemon-reload` | #backend #infra #deploy #bug #blocked
...
[2026-08-26] | Session Software Factory `20260825T215927-438096` : brochure-video pipeline T1 builder généra l'implémentation, mais T1 tester rejeta le déploiement car template systemd crée mais jamais copié vers /etc/systemd/system | DEPLOY_ARTIFACTS et REMOTE_COMMANDS mal alignés — plan déploiement envoie le template à ~/moana sans que la step de finalisation systemd ne le copie/installe en prod | Augmenter le plan avec une commande cp vers /etc/systemd/system (ex. `cp ${TEMPLATE} /etc/systemd/system/ && systemctl daemon-reload`) OU intégrer ansible/salt si disponible pour gérer les artefacts sysadmin | #backend #infra #deploy #bug #todo
