...
Validation complémentaire : une session créée manuellement a été rechargée depuis `D:\temp` ; `/feed` répond 200, `is_logged_in=True` et le cookie `li_at` est présent sans exposer sa valeur. Le premier profil de test a ensuite renvoyé `Rate limit message detected on page`. La session est donc valide mais l’IP/session est limitée ; il faut réduire le volume, attendre le refroidissement et prévoir un statut `rate_limited` plutôt qu’un faux `unresolved`.

Retest contrôlé : le profil répond 200 avec le nom et le contenu visible, mais `PersonScraper` renvoie `Unknown`/0 expérience après contournement du faux positif « try again later ». Le package utilise des sélecteurs obsolètes (`h1`, anciennes classes et listes). La page `/details/experience/` répond pourtant 200 et expose les expériences dans une structure `section`/`a[href*="/company/"]`. Conclusion : session fonctionnelle, package brut non compatible avec le DOM actuel ; un adaptateur de sélecteurs et un détecteur de rate-limit strict sont nécessaires avant intégration.

Correctif retenu : intégrer le package en mode optionnel via une couche `LinkedInCompatScraper`. Elle ne s’active que si `KYC_LINKEDIN_ENABLED=1` et qu’un fichier de session local est présent; elle limite les profils à un par job, refuse les checkpoints/CAPTCHA, utilise les sections DOM actuelles et convertit le résultat en `EvidenceDocument`. Sans session ou en cas de limite LinkedIn, le worker conserve les sources Crawl4AI/SearXNG et ne transforme pas l’échec en identité non résolue.
