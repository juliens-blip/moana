...
## 2026-07-26 — Vraie cause du chevauchement : erreur d'espace d'unités SVG (4e retour utilisateur)

[2026-07-26] CODE/TEST/DEPLOY | Cycles 9/10/11 tous clôturés « tout vert » sans corriger le bug, parce que leur vérif re-dérivait l'hypothèse du clustering et jamais le rayon réellement dessiné par le JSX. Deux causes trouvées et reproduites numériquement avant tout changement : (A) tous les floors (`Math.max(3, r/k)`, `Math.max(0.5, 1/k)`, `Math.max(9, 11/k)`, `Math.max(4, 8/k)`) étaient appliqués APRÈS division par `k`, donc en unités locales dans `<g scale(k)>` — un plancher de `3` y vaut `3·k` pixels écran : rayon de clic mesuré 22px à k=1 mais 48px à k=16, 192px à k=64, 768px à k=256, alors que le clustering garantit seulement 50px entre centres (recouvrement de 46px à k=16, 1486px à k=256, atteint dès le 2e clic puisqu'un clic zoome ×4) ; (B) `bubbleRadius` sans clamp alors qu'un cluster somme ses membres tandis que `maxTotal` est le max par lieu → 34px pour un total de 40, 83px pour 300, contre `MAX_RADIUS`=20, et ce rayon étant réinjecté dans la décision de clustering le blob avalait ses voisins | `lib/geo/bubble-geometry.ts` (nouveau : `bubbleRadius` clampé, `tapRadius` comme source de vérité unique partagée clustering+rendu, `toLocalUnits`), `components/listings/MarketMovementsMap.tsx` (toute la géométrie en px écran, convertie exactement une fois via `toLocal()`, bloc « UNIT CONVENTION »), `tests/bubble-geometry.test.ts` (7 tests), `tasks/market-trends-map/07_unit_space_bug.md` | 45/45 tests, lint 0/0, `tsc --noEmit` propre, `next build` propre ; **chaque assertion vérifiée ROUGE sur le code d'avant correctif** — la 1re version du test anti-chevauchement passait au vert sur le code cassé (grille synthétique à 17 unités d'écart, alors que le bug n'apparaît que pour `50/k < Δ < 6`), remplacée par un jeu réaliste (hubs Côte d'Azur sub-unitaires, poids lourds au rayon plafonné, points coïncidents) | leçon : sur une carte SVG zoomable, pixels et unités locales ne sont pas comparables — un plancher du mauvais côté de la division croît avec le zoom ; et un test de régression jamais observé rouge ne prouve rien

## 2026-07-25 — MAX_ZOOM + spiderfy insuffisants à l'échelle (3e retour utilisateur)

[2026-07-25] LOOP/CODE/TEST | Cycles 9/10 clôturés tout-vert (lint/tsc/build/tests) mais le bug persistait : leur vérif ne rejouait jamais la géométrie réelle à `MAX_ZOOM` ni le spiderfy à l'échelle. Script `_verify-cluster.ts` réparé (corruption octets nuls) et étendu, révèle deux bugs invisibles au testing habituel : `MAX_ZOOM=20` mathématiquement trop bas (aucun hub proche-mais-distinct, ex. Antibes/Monaco 24km, ne pouvait se séparer par zoom) et `SPIDERFY_STEP_PX=26` insuffisant dès qu'un cluster dépasse ~4 membres (41.7px mesuré sur 16 membres, sous le seuil 50px) | `MAX_ZOOM` 20→256, `SPIDERFY_STEP_PX` 26→38 (marge ~22% au-dessus du minimum 34 trouvé par sweep) ; `_verify-cluster.ts` : ALL PASSES CLEAN (4 passes + contrôle géographique réel + sweep) ; agent test-code indépendant vert (lint/tsc/build/26 tests/smoke dev-server) | leçon consignée dans `journalbug.md` : sur une carte à zoom borné, valider `MAX_ZOOM` et le spiderfy contre de VRAIES distances et tailles de cluster, jamais seulement lint/tsc/build ; push demandé explicitement par l'utilisateur

## 2026-07-24 (suite) — Clustering définitif par recouvrement (2e retour utilisateur)

[2026-07-24] LOOP/CODE/TEST | 1er correctif insuffisant : seuil de clustering fixe (26px) ignorait le rayon réel des bulles, donc deux grosses bulles proches mais chevauchantes restaient non fusionnées ; remplacé par `clusterByOverlap` (union-find à point fixe, seuil = somme des rayons réels + marge, recalculé après chaque fusion pour capter les cascades), spiderfy en spirale (angle doré) | lint (0 warning), type-check, build (16/16, 79.5 kB stable), 26/26 tests unitaires (dont le cas exact du bug et la cascade), agent test-code indépendant vert, Graphify réindexé confirmant l'absence de référence résiduelle à l'ancienne fonction | QA tactile réelle reste à faire (pas de navigateur ici) ; push demandé explicitement par l'utilisateur
