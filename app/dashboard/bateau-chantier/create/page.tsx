...
export default function CreateChantierListingPage() {
  ...
  const handleSubmit = async (data: TrackedListingInput) => {
    ...
    try {
      const response = await fetch('/api/bateaux-chantier', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        toast.success('Bateau chantier créé avec succès!');
      ...
      console.error('Error creating chantier listing:', error);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="animate-fade-in-up" style={{ animationDelay: '0ms' }}>
        ...
        <h1 className="text-3xl font-bold text-gray-900 mt-4">
          Ajouter un bateau chantier
        </h1>
        <p className="text-gray-600 mt-2">
          Remplissez les informations du bateau chantier
        </p>
      </div>
