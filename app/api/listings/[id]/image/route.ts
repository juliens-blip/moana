...
export async function POST(
  ...
  try {
    ...
    if (uploadError) {
      console.error('[uploadListingImage] Upload error:', uploadError);
      return NextResponse.json<ApiResponse>(
    ...
    if (existing.image_url && existing.image_url !== imageUrl) {
      ...
      if (oldPath) {
        const { error: removeError } = await supabase.storage
          ...
          .remove([oldPath]);
        if (removeError) {
          console.warn('[uploadListingImage] Failed to remove old image:', removeError);
        }
      }
    }
    ...
    console.error('Error in POST /api/listings/[id]/image:', error);
    return NextResponse.json<ApiResponse>(
