...
export async function POST(
  ...
  try {
    ...
    const filePath = `tracked/bateaux-chantier/${params.id}/${Date.now()}-${Math.random()
      .toString(36)
    ...
    const updated = await updateTrackedListingImage('bateaux_chantier', params.id, imageUrl);
    ...
    console.error('Error in POST /api/bateaux-chantier/[id]/image:', error);
    return NextResponse.json<ApiResponse>(
...
export async function DELETE(
  ...
  try {
    ...
    const updated = await updateTrackedListingImage('bateaux_chantier', params.id, null);
    ...
    console.error('Error in DELETE /api/bateaux-chantier/[id]/image:', error);
    return NextResponse.json<ApiResponse>(
      { success: false, error: 'Erreur serveur' },
      { status: 500 }
    );
